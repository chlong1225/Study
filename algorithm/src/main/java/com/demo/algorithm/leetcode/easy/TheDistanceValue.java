...
public class TheDistanceValue {
    ...
    public int findTheDistanceValue(int[] arr1, int[] arr2, int d) {
        ...
        //2，对arr2中元素进行范围打表
        int preMaxIndex = Integer.MIN_VALUE;
        for (int i = 0; i < n; i++) {
            int minIndex = arr2[i] - d + offset;
            int maxIndex = arr2[i] + d + offset;
            //此时从minIndex～maxIndex都需要置为true
            for (int j = Math.max(minIndex, preMaxIndex); j <= maxIndex; j++) {
                marks[j] = true;
            }
        ...
        for (int i = 0; i < arr1.length; i++) {
            int findIndex = arr1[i] + offset;
            if (findIndex < 0 || findIndex > max - min) {
                count++;
            } else {
                if (!marks[findIndex]) {
                    count++;
