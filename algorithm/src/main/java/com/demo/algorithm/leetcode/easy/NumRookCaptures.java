 ...
 *
 * 在一个8 x 8的棋盘上，有一个白色的车（Rook），用字符'R'表示。棋盘上还可能存在空方块，白色的象（Bishop）以及黑色的卒（pawn），分别用字符 '.'，'B' 和 'p' 表示。
    ...
    public int numRookCaptures(char[][] board) {
        ...
        for (int i = position[1] - 1; i >= 0; i--) {
            if (board[position[0]][i] == 'p') {
                count++;
                break;
            } else if (board[position[0]][i] == 'B') {
                break;
            }
        }
        //3，向右边查找
        for (int i = position[1] + 1; i < n; i++) {
            if (board[position[0]][i] == 'p') {
                count++;
                break;
            } else if (board[position[0]][i] == 'B') {
