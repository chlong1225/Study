...
class DataActivity : BaseActivity<ActivityDataBinding>() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        getViewBinding().btnInsert.setOnClickListener {
            userDao.add(User(120, "haha1", 20))
            userDao.add(User(121, "haha2", 21))
            userDao.add(User(122, "haha3", 20))
            userDao.add(User(117, "haha4", 21))
            userDao.add(User(116, "haha5", 19))
            userDao.add(User(125, "haha6", 20))
            userDao.add(User(124, "haha7", 20))
        }
        ...
        getViewBinding().btnDeleteUser.setOnClickListener {
            userDao.deleteByUser(User(124, "aaa", 34))
            val datas = userDao.queryAll()
            datas.forEach {
